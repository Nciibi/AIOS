...
Confidence scoring quantifies how certain Sou is about each thought step, reasoning chain, and conclusion. It provides per-step confidence estimation, aggregate confidence across chains, calibration against historical accuracy, evidence-based confidence adjustment, and a framework for handling low-confidence situations. Confidence scores drive metacognitive decisions — whether to continue, request more information, qualify an answer, or defer entirely. Confidence thresholds gate actions across all Cognitive OS components.
...
ConfidenceOutput {
  ...
  target_id: string
  target_type: string
  score: number                      // 0.0–1.0 final confidence
...
```typescript
EvidenceQuality {
  average_reliability: number        // 0.0–1.0
  recency_score: number              // 0.0–1.0 (1.0 = current)
  consistency_score: number          // 0.0–1.0 (1.0 = fully consistent)
  completeness_score: number         // 0.0–1.0 (1.0 = no gaps)
  relevance_score: number            // 0.0–1.0 (1.0 = directly relevant)
...
LowConfidenceAction {
  ...
  priority: number                   // 0.0–1.0 urgency
  executed: boolean
}

ConfidenceThresholds {
  auto_act: number                   // ≥ 0.9 — Proceed without review
  normal_act: number                 // ≥ 0.7 — Proceed with normal review
  qualify: number                    // ≥ 0.4 — Qualify answer with uncertainty
  request_info: number               // ≥ 0.2 — Request more information
  defer: number                      // < 0.2 — Defer decision entirely
}
...
ThoughtStep {
  ...
  confidence: 0.82                   ← Per-step confidence
}
```
...
Step confidence = f(
  ...
  complexity                   // More complex → lower confidence
)
```
...
Default: GEOMETRIC — penalizes chains where any step is weak.
```
...
Evidence Quality Assessment
       │
       ├── reliability_factor:   if source_reliability < 0.5 → -0.15
       ├── recency_factor:       if evidence_age > 30 days → -0.05
       ├── consistency_factor:   if conflicts > 0 → -0.10 per conflict
       ├── completeness_factor:  if gaps detected → -0.08
       └── relevance_factor:     if relevance < 0.6 → -0.12
       │
       ▼
...
│                                 │
│  ≥ 0.9 ──► Auto-act (proceed)  │
│  ≥ 0.7 ──► Normal act (review) │
│  ≥ 0.4 ──► Qualify answer      │
│           ("I'm fairly confident but...") │
│  ≥ 0.2 ──► Request more info   │
│           ("I need additional data on X") │
│  < 0.2 ──► Defer               │
...
|-----------|-------|----------|---------|
| auto_act | ≥ 0.9 | Execute without Sou review | Trivial deductions |
| normal_act | ≥ 0.7 | Proceed, include in context for Sou | Routine reasoning |
| qualify | ≥ 0.4 | Add uncertainty qualifier to output | "Based on available evidence..." |
| request_info | ≥ 0.2 | Pause and request additional data | "I need to check X before concluding" |
...
|-------|--------|-------------|
| `COG.CONF.ConfidenceEstimated` | request_id, target_id, score, target_type | Confidence score computed |
| `COG.CONF.StepConfidenceScored` | request_id, step_number, score, decomposition | Individual step confidence assigned |
| `COG.CONF.ChainConfidenceAggregated` | request_id, chain_id, aggregate_score, method | Chain-level confidence computed |
| `COG.CONF.ConfidenceCalibrated` | request_id, score_before, score_after, offset_applied | Calibration adjustment applied |
| `COG.CONF.CalibrationUpdated` | curve_id, new_error, sample_size, domain | Calibration curve updated with new data |
| `COG.CONF.CalibrationReset` | session_id, previous_sample_size | Calibration curve reset |
| `COG.CONF.EvidenceAdjustmentApplied` | request_id, adjustment_value, factor | Evidence quality adjusted confidence |
| `COG.CONF.LowConfidenceTriggered` | request_id, score, threshold, action_type | Confidence below threshold |
| `COG.CONF.LowConfidenceResolved` | action_id, resolved, new_confidence, resolution | Low confidence action handled |
| `COG.CONF.ThresholdChanged` | session_id, threshold_type, old_value, new_value | Confidence threshold updated |
| `COG.CONF.ConfidenceOverride` | target_id, original_score, override_score, reason | Sou overrode a confidence score |

## Invariants

| ID | Invariant | Enforcement |
|----|-----------|-------------|
| COG-CONF-001 | All confidence scores are bounded [0.0, 1.0] | Schema — clamped at domain boundaries |
| COG-CONF-002 | Aggregated chain confidence never exceeds the maximum per-step confidence | Algorithmic — bounded by max |
| COG-CONF-003 | Calibration error is monotonically non-increasing as sample size grows | Algorithmic — only improves with data |
| COG-CONF-004 | Low confidence actions are advisory; Sou decides whether to execute | Architectural — no auto-execution |
| COG-CONF-005 | Evidence adjustment never increases confidence above base + 0.1 | Algorithmic — evidence can only slightly boost |
| COG-CONF-006 | Calibration curves are session-scoped and reset on new session | Algorithmic — per-session state |
| COG-CONF-007 | Every confidence estimate is logged with its full decomposition | Architectural — audit trail required |
| COG-CONF-008 | Threshold comparisons use calibrated scores, not raw scores | Algorithmic — calibration applied first |
...
| Rule | Assessment |
|------|-----------|
| R1 — Modulsingularity | Confidence Estimator only quantifies certainty |
| R2 — Dependency Order | Depends on Evidence Evaluator, Metacognition calibration |
| R3 — DRY | Calibration algorithm defined once; aggregation methods as strategies |
| R4 — Builder Pattern | ConfidenceOutput built from decomposition → calibration → adjustment |
| R5 — Liskov Substitution | Any aggregation method is interchangeable |
| R6 — DI over Singletons | Calibration curves and threshold configs injected |
| R9 — Deterministic | Same inputs produce same confidence at temperature=0 |
| R10 — Simpler Over Complex | Uses linear decomposition model with named factors |
| R13 — Design for Failure | Missing calibration falls back to raw score with penalty |
| R14 — Paved Path | All confidence flows through `estimate` method |
| R15 — Open/Closed | New aggregation methods added by registering with estimator |
