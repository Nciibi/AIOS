...
|-------|--------|-------------|
| `TLS.ValidationPassed` | invocation_id, tool_id, duration_ms | All validations passed |
| `TLS.ValidationFailed` | invocation_id, tool_id, field, code, reason | Validation error detected |
| `TLS.ValidationWarning` | invocation_id, tool_id, field, code, suggestion | Non-fatal validation warning |
| `TLS.ValidationCoerced` | invocation_id, field, from_type, to_type, original_value | Value coerced to expected type |
| `TLS.ValidationClamped` | invocation_id, field, constraint, original, clamped | Value clamped to range |
| `TLS.ValidationDefaultApplied` | invocation_id, field, default_value | Default value used for missing field |
| `TLS.CustomValidatorRegistered` | validator_name, description | New custom validator registered |
| `TLS.CustomValidatorFailed` | validator_name, invocation_id, field, message | Custom validator returned error |
| `TLS.SchemaValidationSkipped` | invocation_id, reason | Validation skipped (e.g., internal tool) |
| `TLS.ValidationMetrics` | total_validations, pass_rate, avg_duration_ms | Aggregated validation metrics |
