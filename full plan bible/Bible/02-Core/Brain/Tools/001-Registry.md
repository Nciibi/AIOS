...
| Document ID | AIOS-BBL-002-TLS-001 |
...
|-------|--------|-------------|
| `TLS.ToolRegistered` | tool_id, name, category, provider, version | Tool registered in the catalog |
| `TLS.ToolDeregistered` | tool_id, provider, category, reason | Tool removed from registry |
| `TLS.ToolHealthChanged` | tool_id, old_status, new_status, consecutive_failures | Tool health transition detected |
| `TLS.ToolStatusChanged` | tool_id, old_status, new_status, version | Tool status lifecycle transition |
| `TLS.ToolUpdated` | tool_id, provider, updated_fields, version | Tool definition modified |
| `TLS.ToolCategoryChanged` | tool_id, old_category, new_category | Tool category reassigned |
| `TLS.RegistryQuery` | query_type, filters, result_count | Registry queried by Sou or system |
| `TLS.RegistrySnapshotTaken` | total_tools, healthy, degraded, unavailable | Registry snapshot generated |
| `TLS.ProviderToolsListed` | provider, tool_ids, count | All tools for a provider listed |
| `TLS.RegistryIntegrityCheck` | total_tools, inconsistencies_found | Periodic registry integrity verified |
