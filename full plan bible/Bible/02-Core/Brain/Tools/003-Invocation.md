...
| Document ID | AIOS-BBL-002-TLS-003 |
...
|-------|--------|-------------|
| `TLS.InvocationCreated` | invocation_id, tool_id, execution_type, source | Invocation request created |
| `TLS.InvocationValidated` | invocation_id, validation_result | Parameter validation passed |
| `TLS.InvocationDispatched` | invocation_id, tool_id, endpoint, deadline | Invocation sent to Runtime SDK |
| `TLS.InvocationCompleted` | invocation_id, status, duration_ms, token_cost | Invocation finished successfully |
| `TLS.InvocationFailed` | invocation_id, error_code, stage, error_message | Invocation failed at a stage |
| `TLS.InvocationTimeout` | invocation_id, deadline, elapsed_ms | Invocation exceeded deadline |
| `TLS.InvocationCancelled` | invocation_id, stage, reason | Invocation cancelled by caller |
| `TLS.InvocationRetrying` | invocation_id, attempt, max_retries, delay_ms | Retrying failed invocation |
| `TLS.InvocationQueued` | invocation_id, reason, queue_position | Invocation queued due to rate limit |
| `TLS.RateLimitExceeded` | tool_id, current_usage, limit, reset_in_ms | Rate limit hit for a tool |
