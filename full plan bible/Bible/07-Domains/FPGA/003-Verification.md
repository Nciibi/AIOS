...
|------|-----------|----------|----------|
| FPGA-VRF-001 | Simulation assertion failure — an assertion evaluates false during simulation | HIGH | Record assertion with stimulus, halt simulation, return waveform at failure point |
| FPGA-VRF-002 | Formal verification inconclusive — tool cannot prove or disprove properties within depth or time | MEDIUM | Increase depth or time limit, switch engine, report unbounded proof unavailable |
| FPGA-VRF-003 | Coverage below threshold — functional coverage is less than 80% minimum | MEDIUM | Return coverage gap report, identify uncovered regions, trigger testbench expansion |
| FPGA-VRF-004 | Bitstream integrity failure — checksum mismatch after bitstream generation | HIGH | Regenerate bitstream, verify toolchain integrity, compare with prior build |
| FPGA-VRF-005 | Readback mismatch — programmed device readback does not match source bitstream | CRITICAL | Report mismatch addresses, trigger device reprogram, isolate hardware fault |
| FPGA-VRF-006 | Gate-level simulation mismatch — gate sim results differ from RTL sim results | HIGH | Compare RTL vs gate waveforms, identify synthesis artifacts, resynthesize with constraints |
| FPGA-VRF-007 | Formal engine timeout — verification exceeds configured time limit | MEDIUM | Partition properties, run with bounded proof, report as partial verification |
...
- **R1 — Single Source of Truth**: SimulationConfig and FormalVerificationPlan are the sole sources for verification setup.
- **R2 — Immutable Event Log**: Every simulation, formal run, assertion check, and verification produces an immutable event.
- **R3 — Capability-Based Authorization**: Only SimulationWorker and SynthesisWorker capabilities may invoke verification engines.
- **R4 — Law of Diminishing Returns**: Coverage above 95% triggers diminishing returns; focus shifts to directed testing.
- **R5 — Deterministic Computation**: Same testbench, seed, and design produces identical simulation results.
- **R6 — Bounded Context**: Verification owns the sim-to-bitstream pipeline; results are broadcast via Events.
- **R9 — Fail-Fast**: Assertion failures stop simulation immediately; formal inconclusive results block signoff.
- **R10 — Audit Trail**: Every verification step, coverage metric, and waiver is logged with timestamp and worker identity.
- **R13 — Defensive Design**: Simulation checkpoints at every assertion boundary; partial coverage results preserved.
- **R14 — Self-Healing**: On formal timeout, FormalEngine retries with partitioned property sets automatically.
- **R15 — Backward Compatibility**: Verification configs maintain versioned migration paths across tool updates.
